    ...
    try {
      ...
      if (emailNote) {
        emailNote.textContent =
          "SCC has received the request. The private Site Factory workflow is generating the preview now; if client email is enabled, the preview link will be emailed after deployment.";
      }
    } catch (error) {
      const json = JSON.stringify(request, null, 2);
      const subject = encodeURIComponent(`Website factory request: ${request.business_name || "New website"}`);
